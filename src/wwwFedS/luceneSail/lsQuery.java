...
public class lsQuery {
	...
	// 对各个数据集最后一次查询
	public static ResultList executeQuery(SailRepositoryConnection curConn, String qs) {
		String lastResult = "";
		...
		try {
			...
			query = curConn.prepareTupleQuery(QueryLanguage.SPARQL, queryString);
			TupleQueryResult result = query.evaluate();
			System.out.println("结果  :  " + result.hasNext());
	...
	public static ResultList dbpediaQuery(String qs) {
		...
		try {
			...
			while (result.hasNext()) {
				...
				if (count == 0) {
					curRes.queries.add(queryString);
				}
				curRes.mappings.add(resList);
				count++;
			}
