			...
			if (str.contains("ls")) {
				...
				try {
					...
					for (int j = 0; j < slist.size(); j++) {
						...
						//txtExport.creatTxtFile("NO."+String.valueOf(j));
						String qs = "";
						for (int i = 0; i < 4; i++) {

							ArrayList<String> qsList = slist.get(j).get(i);
							System.out.println("repository " + i + " of " + qsList.size() + " queries :");
							/*******屏蔽********/
							//out.println("repository " + i + " of " + qsList.size() + " queries :");
							...
							for (int k = 0; k < qsList.size(); k++) {
								...
								//if (k % 10 == 0)
									//System.out.println("======================" + k + "\t" + qs);
								//System.out.println(k+"!!!!"+qs);///////////////////
								//txtExport.creatTxtFile("i"+String.valueOf(i)+"j"+String.valueOf(j)+"k"+String.valueOf(k));
								//txtExport.writeTxtFile(qs);
								String qs1 = qs;
								...
								if (!qs1.equals("")) {
									...
									if (i == 0) {
										...
										new lsQuery();
										// tmpResList.addAllResults(
										// lsQuery.executeQuery(new chebiConnection().getChebiConn(), qs));
										tmpResList.addAllResults(lsQuery.chebiQuery(qs));
									...
									if (i == 1) {
										...
										new lsQuery();
										// tmpResList.addAllResults(
										// lsQuery.executeQuery(new drugbankConnection().getDrugbankConn(), qs));
										tmpResList.addAllResults(lsQuery.keggQuery(qs));
									...
									if (i == 2) {
										...
										new lsQuery();
										// tmpResList.addAllResults(
										// lsQuery.executeQuery(new keggConnection().getKeggConn(), qs));
										tmpResList.addAllResults(lsQuery.drugbankQuery(qs));
									...
									if (i == 3) {
										...
										new lsQuery();
										tmpResList.addAllResults(lsQuery.dbpediaQuery(qs));
										// tmpResList.addAllResults(
										// lsQuery.executeQuery(new dbpediaConnection().getDbpediaConn(), qs));
										long dbpedia_end_time = System.currentTimeMillis();
										timeCollection.get(3).add(dbpedia_end_time - dbpedia_start_time);
									}
								}
								//System.out.println(tmpResList);///////
							}

							// if (j == 44 && qsList.size() != 0) {
							// out.println(i + "------------------" + qsList.get(0) + "\t" + tmpResList);
							// }

							// out.println(tmpResList);

							if (tmpResList.variables.size() == 0 && qsList.size() != 0) {
								finalResList.mappings.clear();
								break;
							}
							...
							if (j % 4 == 0)
				            {
				              txtExport.creatTxtFile("NO." + String.valueOf(j / 4));
				              txtExport.writeTxtFile(qs);
				            }
						...
						total_res_num += finalResList.mappings.size();
						// if (total_res_num >= 200)
						// break;
					}
					...
					for (int i = 0; i < allResLists.size(); i++) {
						...
						if (allResLists.get(i).mappings.size() != 0) {
							out.println(allResLists.get(i));
							//System.out.println(allResLists.get(i));///////////////////
						}
					}
					...
					out.print("first ls: " + (endTime_for_fulltext - startTime) + "ms ");
					out.print("traversing: " + (endTime_for_structquery - endTime_for_fulltext) + "ms ");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				try {
							...
							for (String qs : qsList) {
								...
								if (!qs1.equals("")) {
									if (j == 0) {
										long dbpedia_start_time = System.currentTimeMillis();
										new cdQuery();
										tmpResList.addAllResults(cdQuery.dbpediaQuery(qs));
										long dbpedia_end_time = System.currentTimeMillis();
									...
									if (j == 1) {
										...
										tmpResList.addAllResults(cdQuery.jamendoQuery(qs));
									...
									if (j == 2) {
										...
										tmpResList.addAllResults(cdQuery.linkedmdbQuery(qs));
									...
									if (j == 3) {
										...
										tmpResList.addAllResults(cdQuery.geonamesQuery(qs));
									...
									if (j == 4) {
										...
										tmpResList.addAllResults(cdQuery.swdfoodQuery(qs));
									...
									if (j == 5) {
										...
										tmpResList.addAllResults(cdQuery.nytQuery(qs));
					...
					for (int i = 0; i < allResLists.size(); i++) {
						...
						if (allResLists.get(i).mappings.size() != 0) {
							out.println("the " + i + " final results : " + allResLists.get(i));
						}
					...
					System.out.println("traversing: " + (endTime_for_structquery - endTime_for_fulltext) + "ms");
					System.out.println("final: " + (endTime_for_sparql - endTime_for_structquery) + "ms");
					out.print("fisrt ls: " + (endTime_for_fulltext - startTime) + "ms ");
					out.print("traversing: " + (endTime_for_structquery - endTime_for_fulltext) + "ms ");
					out.println("final: " + (endTime_for_sparql - endTime_for_structquery) + "ms");
