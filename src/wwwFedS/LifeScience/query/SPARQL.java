...
public class SPARQL {
	...
	public void generateExeclist(ArrayList<HashMap<Integer, ArrayList<String>>> queryArray,
			ArrayList<HashMap<String, HashMap<Integer, ArrayList<String>>>> plusArray, String modeName) {
		// System.out.println(plusArray);
		System.out.println("Start Query In Federated RDF System.");
		// 生成合理的sparql查询语句

		if (queryArray.size() == plusArray.size())
			System.out.println("number is correct, sparql query is under making");
			...
			for (int j = 0; j < 4; j++) {
				...
				if (qlist.get(j).size() > 0) {
					for (String str : qlist.get(j)) {
						...
						ArrayList<String> plus = generatePlus(str, plusArray.get(i), modeName);
						ArrayList<String> tmpquerylist = new ArrayList<String>();
						for (int idx = 0; idx < plus.size(); idx++) {
							tmpquerylist.add("SELECT * WHERE{\n" + str + plus.get(idx) + "}");
						}
						// if (!plus.equals(""))
						execList_in.get(j).addAll(tmpquerylist);
					}
				}
	...
	public ArrayList<String> generatePlus(String origin, HashMap<String, HashMap<Integer, ArrayList<String>>> modePlus,
			String modeName) {
		HashMap<Integer, ArrayList<String>> mode = modePlus.get(modeName);
		ArrayList<Integer> kwnum = new ArrayList<>();
		kwnum.addAll(mode.keySet());
		// System.out.println(kwnum);
		ArrayList<String> plusSPARQL = new ArrayList<String>();
		for (Integer kn : kwnum) {
			if (origin.indexOf("k" + String.valueOf(kn)) != -1) {
				plusSPARQL.addAll(mode.get(kn));
			}
		}
		return plusSPARQL;
	}
